...
static void process_debug_commands(void) {
    ...
    if (len > 0) {
        ...
        if (strcmp(rx_buffer, "debug") == 0) {
            ...
            printf("spiffs_check      - Check SPIFFS filesystem status\n");
            printf("spiffs_format     - Format SPIFFS (requires confirmation)\n");
        ...
        else if (strcmp(rx_buffer, "spiffs_check") == 0) {
            printf("\n=== SPIFFS STATUS ===\n");
            ...
            if (ret == ESP_OK) {
                ...
                printf("Used: %zu KB\n", used / 1024);
                printf("Free: %zu KB\n", (total - used) / 1024);
                printf("Usage: %.1f%%\n", (float)used / total * 100.0);
            } else {
                printf("ERROR: Cannot read SPIFFS info: %s\n", esp_err_to_name(ret));
                printf("This may indicate SPIFFS corruption!\n");
            }
            printf("\n");
        }
        else if (strcmp(rx_buffer, "spiffs_format") == 0) {
            printf("\n=== FORMAT SPIFFS WARNING ===\n");
            printf("This will DELETE ALL LOGS!\n");
            printf("Type 'spiffs_format_yes' to proceed\n\n");
        }
        else if (strcmp(rx_buffer, "spiffs_format_yes") == 0) {
            printf("\n=== FORCING SPIFFS FORMAT ===\n");

            // Desmontar SPIFFS primero
            ...
            if (ret == ESP_OK) {
                ...
                if (ret == ESP_OK) {
                    printf("SPIFFS remounted successfully!\n");
                    printf("*** PLEASE RESTART THE ESP32 ***\n");
                } else {
                    printf("ERROR: Failed to remount SPIFFS: %s\n", esp_err_to_name(ret));
                }
            } else {
                printf("ERROR: Failed to format SPIFFS: %s\n", esp_err_to_name(ret));
            }
            printf("\n");
        }
    }
